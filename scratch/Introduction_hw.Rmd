...
## Symbolic AI and Expert System
 ...
 As mentioned at the beginning, one of the way humans are intelligent is our ability to manipulate symbols i.e. language and hence it has also evolved to be a primary way we communicate with each other. 
...
Symbolic was once the dominant paradigm in AI community from the post-War era until the late 1980s.
 ...
 Before we jump into the real applications, let's take some time to look at one of the first high-level programming language used in 1960s AI field - Lisp. 
 
 **Symbolic Computations**

Lisp (historically, LISP) is fully parenthesized prefix notation. Originally specified in 1958, Lisp was created as a practical mathematical notation for computer programs, influenced by the notation of Alonzo Church's lambda calculus. It quickly became the favored programming language for artificial intelligence (AI) research. As one of the earliest programming languages, Lisp pioneered many ideas in computer science, including tree data structures, automatic storage management, dynamic typing, conditionals, higher-order functions, recursion, the self-hosting compiler, and the read–eval–print loop.
 
One of the first program that was deliberately engineered to mimic the grammar skills of a human being is called Logic Theorist developed in 1950-1951 by Allen Newell, Cliff Shaw, and Herbert Simon. The Logic Theorist was a program designed to mimic the problem solving skills of a human being and is called 'the first artificial intelligence program'.

Later in 1959, they developed a system called General Problem Solver (GPS), which is a summary of the thinking activity for people to solve problems. GPS was the first computer program in history which is able to separate its knowledge of problems (rules represented as input data) from its strategy of how to solve problems (a generic solver engine). It has set an promising example for the later symbolic AI projects. 
...
The most successful form of symbolic AI is expert systems, which rely on a network of production rules. It was introduced around 1965 by the Stanford Heuristic Programming Project led by Edward Feigenbaum, who is often called the 'father of expert systems'. Their initial incentive was trying to use machine to perform that tasks where expertise was highly valued and complex, i.e. diagnosing infectious diseases.  As Feigenbaum said, the idea that 

> intelligent systems drive their power from the knowledge they possess rather than from the specific formalisms and inference schemes they use.

It is also worth menitioning that expert systems became some of the first truly successful forms of artificial intelligence software. 

Similar to the previously introduced symbolic AI system, An expert system could be divided into two subsystems: the inference engine and the knowledge base. The knowledge base represents facts and rules. The inference engine applies the rules to the known facts to deduce new facts. Inference engines can also include explanation and debugging abilities. 

As a result, an expert system (ES) could be used to solve problems within specialized domain that ordinary requires a human expert. By mimicking the thinking of the expert, the program can perform the analysis, design, or monitoring, make decisions and many more other tasks. Let's look at some remarkable research and applications in the do,main of expert systems. 

In 1968, MIT has developed a symbolic manipulator system called Macsyma which stands for MAC's SYmbolic MAnipulator. As a mathematician's assistant, it uses heuristics to transform algebraic expressions. After continuous expansion, it can solve more than 600 kinds of mathematical problems, including calculus, matrix operation, solving equation, etc. The successful development of these systems makes the expert system widely concerned by academia and engineering. In 1982, Macsyma was licensed to Symbolics and became a commercial product. In 1992, Symbolics Macsyma was spun off to Macsyma, Inc., which continued to develop Macsyma until 1999. That version is still available for Microsoft's Windows XP operating system.

The first expert system to be used in a design capacity for a large-scale product was the Synthesis of Integral Design - SID software program developed in 1982. The input to the software was a set of rules created by a couple of expert logic designers. SID can expand the rules and generate software logic synthesis routines many times in the design that exceeds the capabilities of the experts themselves. 

Up until the current time, expert system is still in use of many leading business application suits vendors such as SAP Oracle. Typically, a expert system is used as a way of specifying business logic - rule engines are no longer simply for defining the rules but rather for any type of comples, volatile and critical business logic. They often go hand in hand with business process automation and integration environments. 

However, as time passes by, symbolic AI is no longer at the center of the AI research. Nevertheless, it was intended to produce general, human-like intelligence in a machine, where as most modern research is directed to different specific sub-problems i.e. perception. Research into general intelligence is now a wildly studied topic in the sub-field of artificial general intelligence.

**Symbolic AI vs Deep Learning **
Currently, whenever speaking about AI many people will natually think of deep learning when they hope for the future. 

Apart from the framework used by symbolic AI which is initially designed to formulate output based on the inputs that were represented by symbols, the representation done by deep learning could be considered as 'fuzzy logic' which is wildly used when uncertainty is involved in the data. 

However, there are some obvious flaws of deep learning such as its lack of model interpretability and the amount of data that deep neural networks require so as to learn. 

It brough our attention back to symbolic AI. How can we fuse the ability of deep neural network to learn probabilistic correlation from scratch alongside abstract and higher-order concepts? Research in deep symbolic learning has shown to enable deep neural network to manipulate, generate and otherwise cohabitate with concepts expressed in strings of characters, could extend its explainability. 
...
Another opponents of the symbolic approach include roboticists i.e. Rodney Brooks, who aims to produce antonomous robots without symbolic representation and computational intelligence researchers for which we will expand more in the later chapter. 


## Computation and Machines

We have discussed about the origin of AI. The start of AI could be considered as a human self-discovery journey. It began with a philosophical query about our existence, however, the development of AI has never departed from the physical carrier.  

Ever since we deemed reasoning as one of our human specific properties, many scientists have set out to mechanize this process.
...
In c. 1050–771 BC, the south-pointing chariot was invented in ancient China. It was the first known geared mechanism to use a differential gear, which was later used in analog computers. Later around 2nd century BC Chinese also invented a more sophisticaed abacus which is known as the Chinese abacus.
...
The Antikythera mechanism is believed to be the earliest mechanical analog "computer", according to Derek J. de Solla Price. It was designed to calculate astronomical positions. It was discovered in the Antikythera wreck off the Greek island of Antikythera, between Kythera and Crete, and has been dated to 100 BC.

 Raymon Lull (1230-1315), a Spanish courtier and later converted monk and apologist, is the first person in history known to have devised a "logic" machine—a machine that computes logical proofs rather than doing arithmetic. 
...
Next, we move ahead several centuries and to Scotland. John Napier was born in 1550 near Edinburgh. He discovered that the multiplication and division of numbers could be performed by the addition and subtraction, respectively, of the logarithms of those numbers. It was at this point that he designed his 'Napier's bones', an abacus-like device that greatly simplified calculations that involved multiplication and division.
 
 After John Napier, there followed a period of considerable progress by scientists in making calculating tools. 
 
 As mentioned earlier, Blaise Pascal (1623-1662), an 18-year-old in 1642, invented what has come to be known as the "Pascaline" (his name was "numerical wheel calculator") to facilitate his father's work as a French tax collector based in Paris.  The numerical wheel calculator, or Pascaline, consisted of a rectangular box employing eight movable cogwheels or dials exploiting base ten to perform addition of sums up to eight figures long. 
 
 His successor Gottfried Wilhem von Leibniz (1646-1716) studied Pascal's original notes and drawings to created a machine that  improved upon the Pascaline.   Leibniz's machine could add, subtract, multiply and divide. Leibniz modified the machine to include a stepped-drum gear design, called the Leibniz Wheel.  
 
The apex of this early era of formal computing can be seen in the difference engine and its successor the analytical engine, both by Charles Babbage and Ada Lovelace. The analytical engine combined concepts from his work and that of others to create a device that if constructed as designed would have possessed many properties of a modern electronic computer. 
Babbage's devices could be reprogramed to solve new problems by the entry of new data, and act upon previous calculations within the same series of instructions. Ada Lovelace took this concept one step further, by creating a program for the analytical engine to calculate Bernoulli numbers, a complex calculation requiring a recursive algorithm.

**Analoge Computers**

The earliest computing  machines in general were not digital but analog. In analog representation, properties of the representational medium ape properties of the represented state-of-affairs. 
While Babbage and Lovelace labored to implement digital computing instruments, James Thomson developed an analogue computer in the form of a mechanical integrator to predict the tides using harmonic analysis. Thomson completed his work between 1861 and 1864.


**Logic Machines**
Right around the same time, Augustus de Morgan and George Boole showed that propositions in logic could be treated algebraically. This led to a breakthrough for designing a machine that could mechanically proces logical propositions using these laws. 

The best known of them was the *Lgoci piano* designed by Willian Jevons. 

Though logic machines did not have any practical significance but they reinforced the relationship between logic and computing. They also paved the way for an important theoretical paper on computing later wrote by Alan Turing. 

**The Universal Turing Machine**

Around 1930s, Alan Turing invented the princile of the modern computer in which he desribed an abstract digital computing machine. 

Based on his design, the machine should have a limitless memeory and a scanner that moves back and forth through the memory, symbol by symbol, reading what it finds and writing futhre symbols. 

This computing machine of 1936 is now known as the universal Turing machine. 

Besides that, Alan Turing also came up with the futuristic view of a computing machins that can 

* Learn from experience and 

* solve problems by means of searching through the space of possible solutions, guided by rule-of-thumb principles (the modern term for this idea is called 'heuristic search')
...
The first fully functioning eletronic digital computer was Colossus, used by the Bletchley Park cryptanalysts from 1944.  The Colossus was a joint effort by many people. The Colossus made Boolean comparisons between two strings and was used specifically to decode German messages.


The ENIAC (Electronic Numerical Integrator And Computer) was the first electronic general-purpose computer built in the US, constructed at the Moore School of Elebrical Engineering and announced to the public in 1946. It was Turing-complete, digital, and capable of being reprogrammed to solve a full range of computing problems. 

In 1944, John von Neumann joined the ENIAC group. He had become ‘intrigued' with Turing's universal machine while Turing was at Princeton University during 1936–1938. At the Moore School, von Neumann emphasised the importance of the stored-program concept for electronic computing, including the possibility of allowing the machine to modify its own program in useful ways while running. 

Von Neumann has made some significent contribution to the concept of a high-speed stored-program digital computer widely known through his writings and public addresses.Von Neumann investigated the question of whether modelling evolution on a digital computer could solve the complexity problem in programming. As a result, he designed a self-reproducing computer program is considered the world's first computer virus, and he is considered to be the theoretical father of computer virology and the machine is referred as ‘von Neumann machines’.
...
Around 1970s, Intel released the first microprocessor which was a special integrated circuit that was able to process four bits of data at a time. 

The 1980s brought about significant advances with microprocessor that greatly impacted the fields of engineering and other sciences. The Motorola 68000 microprocessor had a processing speed that was far superior to the other microprocessors being used at the time. 

Lisa was the first personal computer with graphical user interface (GUI) that was sold commercially, she ran on the Motorola 68000, dual floppy disk drives, a 5 MB hard drive and had 1MB of RAM .

In 1983, Apple released Lisa which was the first personal computer with graphical user interface (GUI) that was sold commercially.




# Neurobiolgoy + Learning 


## Neuroscience 

As we shall see from the previous discussion, the most basic inqury behind AI is our question about our existence and in search of the meaning of human intelligence. 

From this perspective, we can think of the AI research and neuroscientists are actually dealing with a set of very resembled age-old goal: to understand the essence of intelligence. 

As Aude Oliva siad: 


"The fundamental questions cognitive neuroscientists and computer scientists seek to answer are similar, they have a complex system made of components - for one, it's called neurons and for the other it's called units."


Despite different tools and questions that are explored by both disciplines, they constantly learn from each other and inspired each other.

Here let's take a look at this long and interwined hishostry between AI and neuroscience. 


As we call know that human brain might be the best representation of intelligence known, with unsurpassed ability to do complex reasonining, real-time perception and decision making with such a rich and dynamic world we live in. 


Human brain undoubfully is one of most elegant instruments that many AI researchers and computer scientists try to imitate and learn from. 


Most recent advances in Nueroscience certainly provide the guide for AI researchers to pursue a more brain-inspired apporach to AI.

But how?


We all know that neuroscience is the subject of biology that focuses on the study of anatomy and phsiology of human brain, including structures, neurons, and molecules to help us better depict the mechanisms, functions, and systems that are behind our cognitive behaviors.


One of the earliest attempt to bridge this 2 disciplines is from Warren McCulloch and Wlter Pitts. Around 1943, they published a paper "A logical Calculus of the ideas immanent in Nervous Activity".

Warren McCulloch is a neuroscientist and Walter Pitts is a logician. Their collaboration gave birth to the first computational theory and prototype of mind and brain. Their paper is also considered as the starting point in neuro network research. 

What was novel in their paper was a theory that employed logic and mathematical notion of computation - introduced by Alan Turing (1936) - to explain how neural mechanism might realize mental functions and unlock the key to our reasoning and intelligence. 


Let's take a more recent example - Deep learning, a branch of AI, it also has the deep root in the science of brain. Deep learning is also known as Artificial Neural Networks which was designed to emulate the structure of the neurons. Beyond the similarities of the structured architectures including dedicated systems for attention, recursion and special forms of long- and short- term memory storage, there are also plenty of other techniques and new features were invented in the deep learning realm that are inspired by neuroscience.

For example, the mechanism of deep networks when applied to Imagenet dataset seems to mimic not only the recognition perfomrance, but also the tuning properties of the real neurons in our cortical areas. The original idea of this came from basic neuroscience work in visual cortex by Hubel and Wiesel who recorded in 1960s how the simple and complex celss in the primary visual cortext work. Their work suggests a hierarchy of simple, complex and hyper-complex cells are structured in a sequence of layers of units to support more and more xomples features and greater invariance. 


Nevertheless to say the framework i.e. transfer learning in contemporary AI research, to be able to deal with the novel scenarios with the existing knowledge. Human have nailed this task since we can operate freely on different type of eletronic devices with ease and deal with different social dynamics with our existing social skills. 


Even in the field like dream and imagination that is so unique to human also shed light on the recent AI research i.e. DeepDream and in turn help us to better understand who we are. 


> With so mcuh at stake, the need for the filed of nueroscience and AI to come together is now more urgent than ever before.
